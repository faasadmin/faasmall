					...
					<view class="specification-content">
						...
						>
							...
							<view class="item-wrapper">
								<view
									class="item-content"
									v-for="(item_value, index2) in item.value"
									:key="index2"
									:class="[
									...
									@click="skuClick(item.name,item_value, index1, index2)"
								>
									{{ item_value.value }}
								</view>
							</view>
...
export default {
	...
	emits: ["update:modelValue", "input", "update-goods", "open", "close", "add-cart", "buy-now","select-array","select-whole","select-good"],
	props: {
	...
	methods: {
		...
		// sku按钮的点击事件
		skuClick(name,value, index1, index2) {
			let that = this;
			if (value.ishow) {

				if (that.selectArr[index1] != value.value) {
					that.$set(that.selectArr, index1, value.value);
					that.$set(that.selectWhole, index1, name + ':' + value.value + ' ');
					that.$set(that.subIndex, index1, index2);
				} else {
					that.$set(that.selectArr, index1, "");
          that.$set(that.selectWhole, index1, "");
        ...
        that.$emit("select-good", that.selectShop);
        that.$emit("select-array", that.selectArr);
        that.$emit("select-whole", that.selectWhole);
			} else {
				that.selectShop = {};
			}
		},
		...
		checkInpath(clickIndex) {
			...
			for (let i = 0, len = specList.length; i < len; i++) {
				...
				let len2 = specList[i].value.length;
				for (let j = 0; j < len2; j++) {
					if (that.subIndex[i] != -1 && j == that.subIndex[i]) {
						continue;
					}
					let choosed_copy = [...that.selectArr];
					that.$set(choosed_copy, i, specList[i].value[j].value);
					...
					if (that.shopItemInfo.hasOwnProperty(that.getArrayToSting(choosed_copy2))) {
						specList[i].value[j].ishow = true;
					} else {
						specList[i].value[j].ishow = false;
					}
				}
			}
      ...
      debugger
			if (specListArr.length === 1) {
				let specList = specListArr[0].value;
				for (let i = 0; i < specList.length; i++) {
					let sku = that.getListItem(skuList, that.skuArrName, [specList[i].value]);
					if (sku) {
		...
		selectSku(obj = {}) {
			...
			if (skuArr && specListArr.length === skuArr.length) {
				...
				for (let index = 0; index < skuArr.length; index++) {
					...
					let specList = specListArr[index].value;
					let index1 = index;
	...
	computed: {
		...
		isManyCom() {
			...
			if (
				...
				goodsInfo[specListName][0].value.length === 1 &&
		...
		priceCom() {
			...
			if (selectShop[skuIdName]) {
				str = that.priceFilter(selectShop.sellingPrice);
			} else {
				let skuList = goodsInfo[skuListName];
				if (skuList && skuList.length > 0) {
					let valueArr = [];
					skuList.map((skuItem, index) => {
						valueArr.push(skuItem.sellingPrice);
					});
