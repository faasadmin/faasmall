...
<template>
  <view class="draw-money-wrap u-p-b-30">
    <view class="head-box">
      ...
      <view class="wallet-num-box u-flex u-col-center u-row-between">
        <view class="">
          ...
          <view class="wallet-num">{{ memberInfo.balance || '0.00' }}</view>
        </view>
        <button class="u-reset-button log-btn"
                @tap="$Router.push({ path: '/pages/common/wallet/recharge_log' })">充值记录</button>
      </view>
    </view>
    <!-- 充值输入卡片-->
    <view class="draw-card">
      ...
      <!-- 快捷充值 -->
      <view class="top-up-content u-p-b-30">
        <view class="card-title">快捷充值</view>
        <view class="u-flex u-flex-wrap">
          <view class="top-up-card u-flex u-row-center u-col-center u-m-b-30 u-m-r-30"
                :class="{'top-up-active':curMoneyCard === index}" v-for="(item,index) in wallet.customAmountData"
                :key="index" @tap="selectCard(index,item)">
      ...
      <view v-if="deliverAmount || deliverIntegral" class="u-flex row-center-center">
        <text v-if="deliverAmount">赠送{{deliverAmount}}元</text>
        <text v-if="deliverIntegral">+{{deliverIntegral}}积分</text>
      </view>
...
export default {
  ...
  computed: {
    ...mapGetters(['memberInfo','walletData'])
  },
  async onLoad() {},
  onPageScroll(e) {
    this.navBackground = e.scrollTop > 50 ? '('+ this.$FILE_URL + '/file/img/wallet/withdraw_head_bg.png) no-repeat top / 100% auto': 'none';
  },
  methods: {
    // 充值输入框
    onTopupInput(e) {
    ...
    selectCard(index,item) {
      ...
      this.money = this.wallet.customAmountData[index].amount;
      this.deliverAmount = item.deliverAmount;
      this.deliverIntegral = item.deliverIntegral;
    },
    confirmTopup() {
      const that = this;
      if (this.money <= 0) {
